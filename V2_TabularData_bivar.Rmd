...
base2=ggplot(df.C, aes(x = reorder(Crime, Percent), y = Percent ) ) 
#base3=ggplot(df.T, aes(Daytime,Crime))

lolliplot1=base2 + geom_segment(aes(y = 0, 
                                   x = reorder(Crime, Percent), 
                                   yend = Percent, 
                                   xend = Crime,
                                   size = Percent), color = "black")
lolliplot2 = lolliplot1 + theme(axis.text.x = element_text(size=6,angle = 45)) 
...
base  = ggplot(df.C, aes(x = Daytime, y = reorder(Crime, -Percent), fill = Percent)) 
heat1 = base +  geom_tile() 
heat2 = heat1 +scale_fill_gradient(low = "yellow",high = "purple")
heat2
```
